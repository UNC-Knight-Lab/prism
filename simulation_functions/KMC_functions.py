...
class ThermalRAFTSequenceEnsemble():
    ...
    def _initiate_with_R(self, mmol_feed, num_monomers, monomer_index, R_mmol, idx, delta, uncapped_chains):
        ...
        return mmol_feed, R_mmol, uncapped_chains
    ...
    def _get_Rmax(self, mmol_feed, rate_matrix, capped_chains, uncapped_chains, delta):
        max_Rs = max(np.max(rate_matrix, axis=0) * mmol_feed)
        max_equilib = max(k_tr*(capped_chains*delta)*(uncapped_chains*delta), k_tr*(capped_chains*delta), k_tr*(uncapped_chains*delta))
        return max(max_Rs, max_equilib)

    def _growth_move(self, mmol_feed, num_monomers, monomer_index, last_monomer, r_matrix, capped_chains, CTA_mmol, R_mmol, delta, R_max):
    ...
    def _capping_move(self, active_chains, delta, R_max):
        P_uncap = k_uncap*active_chains*delta / R_max
    ...
    def _first_capping(self, idx, CTA_mmol, R_mmol, delta, capped_chains, uncapped_chains, capped_index):
        if CTA_mmol - delta > 0:
        ...
        return CTA_mmol, R_mmol, delta, capped_chains, uncapped_chains
    ...
    def _run_first_block(self, mmol_feed, num_monomers, n_impure_chains, r_matrix, conversion):
        ...
        while np.max(self.lengths) <= self.max_DP: #capped_chains <= frac_capped*self.n_chains:
            ...
            while (R_mmol - delta) >= 0:
                ...
                if chain == False:
                    ...
                    mmol_feed, R_mmol, uncapped_chains = self._initiate_with_R(mmol_feed, num_monomers, monomer_indexes, R_mmol, chain, delta, uncapped_chains)
            
            chain = random.choice(np.arange(0,(self.n_chains + n_impure_chains)))
            R_max = self._get_Rmax(mmol_feed, r_matrix, capped_chains, uncapped_chains, delta)
            ...
            if self.chain_status[chain] == 0:
                ...
                new = self._growth_move(mmol_feed, num_monomers, monomer_indexes, last_monomer, r_matrix, capped_chains, CTA_mmol, R_mmol, delta, R_max)
                ...
                if new == num_monomers + 1: # chain caps with another chain
                    ...
                    CTA_mmol, R_mmol, delta, capped_chains, uncapped_chains = self._first_capping(chain, CTA_mmol, R_mmol, delta, capped_chains, uncapped_chains, capped_index)
                ...
                cap_move = self._capping_move(uncapped_chains, capped_chains, R_max)

                if cap_move == 'uncapping':
            ...
            if attempt % 100000 == 0:
                print(1 - np.sum(mmol_feed) / np.sum(orig), capped_chains / (n_impure_chains + self.n_chains), capped_chains, uncapped_chains, R_max)
    ...
    def run_statistical(self, feed_ratios, initiator, rate_matrix, c_tr, conversion = None):
        ...
        self.sequences = np.zeros((self.n_chains + n_impure_chains, self.max_DP))
        ...
        capped_chains, dead_index = self._run_first_block(feed_ratios, num_monomers, n_impure_chains, rate_matrix, conversion)
...
class FRP():
    ...
    def _growth_move(self, mmol_feed, num_monomers, monomer_index, last_monomer, r_matrix):
        rates_add = np.zeros((num_monomers,3)) # first column is normalized rate, second is addition, third is sum
    ...
    def _run_first_block(self, mmol_feed, num_monomers, r_matrix, conversion):
        delta = 1 / self.n_chains
        ...
        while np.max(self.lengths) <= self.max_DP:
            ...
            new = self._growth_move(mmol_feed, num_monomers, monomer_indexes, last_monomer, r_matrix)
            mmol_feed = self._growth_update(mmol_feed, new, chain, delta, monomer_indexes, num_monomers)
    ...
    def run_statistical(self, feed_ratios, rate_matrix, conversion = None):
        ...
        self._run_first_block(feed_ratios, num_monomers, rate_matrix, conversion)
...
class PETRAFTSequenceEnsemble():
    ...
    def _growth_move(self, mmol_feed, num_monomers, monomer_index, last_monomer, r_matrix, capped_chains, R_mmol, Z_mmol, delta):
        ...
        # chain caps with another chain
        rates_add[num_monomers,0] = (capped_chains*delta)*k_tr
        rates_add[num_monomers,1] = num_monomers + 1
        rates_add[num_monomers,2] = sum(rates_add[:,0])

        # chain caps with Z_group
        rates_add[num_monomers+1,0] = Z_mmol*k_trZ
        rates_add[num_monomers+1,1] = num_monomers + 2
        rates_add[num_monomers+1,2] = sum(rates_add[:,0])
    ...
    def _uncapping_move(self, pc_mmol, capped_chains, delta): # chain uncaps with other chain or through photocatalyst
        ...
        rates_cap[0,0] = k_tr*num_uncapped*delta
        rates_cap[0,1] = sum(rates_cap[:,0])

        # chain uncaps with photocatalyst
        rates_cap[1,0] = k_tr*pc_mmol
    ...
    def _capping_type(self, Z_mmol, capped_chains, delta):
        ...
        # chain caps with another chain
        rates_cap[0,0] = k_tr*capped_chains*delta
        rates_cap[0,1] = sum(rates_cap[:,0])

        # chain caps with Z group
        rates_cap[1,0] = k_trZ*Z_mmol
        rates_cap[1,1] = sum(rates_cap[:,0])
    ...
    def _run_unaugmented(self, mmol_feed, num_monomers, pc, r_matrix, conversion):
        delta = 1 / self.n_chains
        ...
        while np.max(self.lengths) <= self.max_DP:
            ...
            if attempt % 50000 == 0:
                print( 1- np.sum(mmol_feed) / np.sum(orig), 1 - (capped_chains / self.n_chains))
    ...
    def run_full_statistical(self, feed_ratios, photocatalyst, rate_matrix, C_tr, conversion = None):
        self._get_rate_constants(C_tr, rate_matrix)
        num_monomers = feed_ratios.shape[0]
        self.max_DP = int(np.sum(feed_ratios) + 500)
        self.sequences = np.zeros((self.n_chains, self.max_DP))
        ...
        capped_chains, dead_index = self._run_unaugmented(feed_ratios, num_monomers, photocatalyst, rate_matrix, conversion)

        return self.sequences    
      
    def run_statistical(self, feed_ratios, photocatalyst, rate_matrix, C_tr, conversion = None):
        self._get_rate_constants(C_tr, rate_matrix)
        num_monomers = feed_ratios.shape[0]
        self.max_DP = int(np.sum(feed_ratios) + 50)
        self.sequences = np.zeros((self.n_chains, self.max_DP))
