...
class PetRAFTSequenceEnsemble():
    ...
    def _Z_group_capping(self, idx, Z_mmol, delta, capped_index):
        if Z_mmol - delta > 0:
            self.sequences[idx, self.lengths[idx]] = capped_index
            Z_mmol -= delta
            self.lengths[idx] += 1
            self.chain_status[idx] = capped_index
    ...
    def _system_moves(self, pc, CTA_mmol, R_mmol, Z_mmol, delta, r_ij, mmol_feed, monomer_index, capped_chains, uncapped_chains):
        ...
        rates[idx,0] = self.k_tr * 100 * sum(mmol_feed) * R_mmol
        ...
        rates[idx,0] = self.k_tr * 100 * CTA_mmol * pc
        ...
        while np.max(self.lengths) <= self.max_DP:
            ...
            if move == self.num_monomers + 1: # chain caps any mechanism
                ...
                if cap_move == 'capped by chain':
                    ...
                    Z_mmol = self._Z_group_capping(chain, Z_mmol, delta, capped_index)
    ...
    def run_statistical(self, feed_ratios, pc, rate_matrix, c_tr, conversion = None):
