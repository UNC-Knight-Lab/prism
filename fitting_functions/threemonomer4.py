...
class ThreeMonomerPETRAFTKineticFitting():
    ...
    def _loss(self, pred_X, pred_F, i):
        ...
        ssr = np.sum((self.exp_data.iloc[:,i] - y_interpolated) ** 2)
        grad_diff = np.sum((grad_exp - grad_sim) ** 2)
        lambda_ = 0.1
    ...
    # def extract_rates(self, t_max, bounds):
    #     self.t_max = t_max
    ...
    def extract_rates(self, t_max, bounds):
        ...
        k = minimize(fun=self._objective1, x0=k, method='L-BFGS-B', bounds=bounds)
        print("Converged rates are", k.x)
